    ...
    <footer className="mt-24 border-t border-border bg-[#000000]">
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 md:py-10">
        <div className="mb-8 flex flex-col gap-5 border-b border-border pb-8 md:flex-row md:items-end md:justify-between">
          ...
          <a href={profile.links.email} className="inline-flex items-center gap-2 border border-primary px-3 py-2 text-xs font-semibold text-primary hover:bg-primary hover:text-primary-foreground">Start a conversation <ArrowUpRight className="h-3.5 w-3.5" /></a>
        </div>
        <div className="grid gap-8 md:grid-cols-[1fr_1.4fr_0.8fr]">
          <div>
          ...
          <div className="flex gap-2 md:justify-end">
            <a href={profile.links.github} target="_blank" rel="noreferrer" aria-label="GitHub" className="grid h-8 w-8 place-items-center border border-border text-muted-foreground hover:border-primary hover:text-primary"><Github className="h-4 w-4" /></a>
            <a href={profile.links.linkedin} target="_blank" rel="noreferrer" aria-label="LinkedIn" className="grid h-8 w-8 place-items-center border border-border text-muted-foreground hover:border-primary hover:text-primary"><Linkedin className="h-4 w-4" /></a>
            <a href={profile.links.email} aria-label="Email" className="grid h-8 w-8 place-items-center border border-border text-muted-foreground hover:border-primary hover:text-primary"><Mail className="h-4 w-4" /></a>
          </div>
        </div>
        <div className="mt-10 flex flex-col gap-2 border-t border-border pt-4 font-mono text-[10px] uppercase tracking-widest text-tertiary sm:flex-row sm:items-center sm:justify-between">
          ...
          <span>built for careful readers <ExternalLink className="inline h-3 w-3" /></span>
        </div>
      </div>
    </footer>
  );
