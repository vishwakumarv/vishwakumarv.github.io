...
export default function ProjectDetail() {
  ...
  const title = `${p.name} — Case Study · Vishwa Kumar`;

  return (
    <article className="mx-auto w-full max-w-4xl px-4 py-12 sm:px-6 md:py-24">
      <Helmet>
        <title>{title}</title>
        <meta name="description" content={p.tagline} />
        <meta property="og:title" content={`${p.name} — Case Study`} />
        <meta property="og:description" content={p.tagline} />
        <meta property="og:type" content="article" />
        <meta property="og:url" content={`/projects/${p.slug}`} />
        <link rel="canonical" href={`/projects/${p.slug}`} />
      </Helmet>
      ...
      <header className="mt-6">
        ...
        <h1 className="mt-2 font-display text-3xl font-semibold tracking-tight break-words mobile-text-balance md:text-5xl">{p.name}</h1>
